    ...
    if users:
        ...
        for user in users:
            user_tags += f'<div class="user-piece"> \
                                ...
                                <button class ="black-button" id="{user[0]}" onclick="blockUser(this.id)"> Block \
                                </button> \
                            </div>'
    else:
...
def delete_other_user(current_user: Annotated[User, Depends(get_current_user)], userId: str = Path()):
    if not current_user.is_admin:
        raise HTTPException(status_code=status.HTTP_403_FORBIDDEN,
                            detail='Available only for administrators')
